# FHNW Security Lab Experimental 🧪

> Experimental code, tests, and work-in-progress from the [FHNW Security Lab](https://github.com/FHNW-Security-Lab)

---

## ⚠️ Notice

This organization contains **experimental and unfinished code**. Projects here may be:
- Work-in-progress implementations
- Proof-of-concept experiments
- Testing grounds for new ideas
- Unstable or incomplete

**Use at your own risk** - code here is not production-ready.

## 🔬 What You'll Find Here

- **Experimental tools** and prototypes
- **Test implementations** and sandbox projects
- **Work-in-progress** research code
- **Proof-of-concepts** before they mature

## 🔗 Related

- 🔬 [FHNW Security Lab](https://github.com/FHNW-Security-Lab) - Our main research organization
- 🎓 [FHNW Security Education](https://github.com/FHNW-security-education) - Educational materials
- 📚 [FHNW Security Lab Student Projects](https://github.com/FHNW-Security-Lab-Student-Projects) - BSc/MSc projects
- 🌐 [FHNW](https://www.fhnw.ch) - University of Applied Sciences and Arts Northwestern Switzerland

---

*Securing tomorrow's technology today through experimentation.*
